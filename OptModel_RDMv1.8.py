...
projects = pd.read_csv("projects_final_v3.csv", delimiter=";")
...
def run_model_with_json(json_path, result_csv_path, result_plot_path):
    ...
    objective_values = []  # Resetear la lista de valores de función objetivo para cada ejecución
    
    # Cargar los datos del modelo desde el archivo JSON
    with open(json_path) as f:
        model_data = json.load(f)
    
    # Crear el wrapper de Platypus utilizando los datos del modelo
    wrapper = CustomPlatypusWrapper(model_data)

    # Ajustar el número de variables binarias que se usarán en la optimización
    n_decision_variables = len(wrapper.problem.types)  # Establecer el número correcto basado en el modelo Pywr
    # Adjust the number of binary decision variables based on the length of each TimeWindowBinaryParameter
    # Modify wrapper to ensure correct length for binary variables
    ...
    # Configurar el algoritmo genético (GA)
    ga = platypus.GeneticAlgorithm(
        wrapper.problem,  # Problema a optimizar
        population_size=100,  # Tamaño de la población
        offspring_size=100,   # Número de hijos generados por iteración
        selector=platypus.TournamentSelector(2),  # Selección por torneo
        comparator=platypus.ParetoDominance(),  # Criterio de dominancia de Pareto
        variator=custom_HUX(probability=1.0),
        evaluator=evaluator
    )

    # Ejecutar optimización con GA
    ...
    # Graficar y guardar la evolución de la función objetivo
    plt.figure()
    plt.plot(objective_values)
    plt.xlabel("Iteración")
    plt.ylabel("Valor de la Función Objetivo")
    plt.title("Evolución de la Función Objetivo en cada Iteración")
    plt.savefig(result_plot_path)
    plt.close()

    # Incluir los comentarios y prints originales
